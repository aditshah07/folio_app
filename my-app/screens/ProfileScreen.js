...
export default class ProfileScreen extends React.Component {
              ...
              urls.map(url => axios.get(url)
                .then((response) => {
                  ...
                  if (response.status === 200) { 
                    ...
                    imageUrls.push({imageURL:responseBody.items[0].volumeInfo.imageLinks.smallThumbnail, title: title});
                    this.setState({
                      imageUrls: imageUrls
                    });
                  } 
  ...
  var fetchOptions = {
    //method: 'PUT',
    headers: {'X-Okapi-Tenant': 'diku', 'Content-Type': 'application/json', "X-Okapi-Token": this.props.navigation.state.params.token},
